...
parser.add_argument('--latent_sampling', help = 'Number of samples in latent space per dimension',type = int, default = 50)
parser.add_argument('--ignore', help = 'Ignore tiles with fewer than this particles',type = int, default = 10)
parser.add_argument('--filter', help = 'Choice of filter to use in the filtered backprojection', type = str, default = 'ctf' )
parser.add_argument('--random_half_decoder', help = 'reconstruct half sets with decoders trained on random half sets', action = 'store_true')
parser.add_argument('--reconstruction_area', help='directory of mask file for local reconstruction', type= str)
parser.add_argument('--noise', help='add noise in training of the inverse deformation field', type= str, default = False)
...
cons_model.ampvar = torch.nn.Parameter(torch.zeros(n_classes,decoder_half1.n_points).to(device),requires_grad = False)
...
if args.reconstruction_area:
    ...
    sm_bin_mask = torch.nn.functional.conv3d(bin_mask.unsqueeze(0).unsqueeze(0),torch.ones(1,1,sm_width,sm_width,sm_width).to(device)/(sm_width**3),padding = 'same')
...
class fwd_deformation_interpolator:
    ...
    def compute_field(self,values):
        ...
        values = values.cpu()
        disp = self.coeff[:,0,None]*values[self.pos[:,0]]+self.coeff[:,1,None]*values[self.pos[:,1]]+self.coeff[:,2,None]*values[self.pos[:,2]]
        disp = torch.tensor(self.grid) - disp + torch.tensor(self.grid)
        values = torch.tensor(self.grid) - values + torch.tensor(self.grid)
        dispM[self.int_grid[:,2],self.int_grid[:,1],self.int_grid[:,0],:] = values[:,:].float()
...
class fwd_deformation_interpolator_ds:
    ...
    def compute_field(self,values):
        ...
        values = values.cpu()
        values = torch.tensor(self.grid) - values + torch.tensor(self.grid)
        dispM[0,:,self.int_grid[:,2],self.int_grid[:,1],self.int_grid[:,0]] = values.movedim(0,1).float()
...
class bwd_deformation_interpolator:
    ...
    def compute_field(self,points,values):
        ...
        values = values.cpu()
        disp = coeff[:,0,None]*values[pos[:,0]]+coeff[:,1,None]*values[pos[:,1]]+coeff[:,2,None]*values[pos[:,2]]
        dispM[self.int_grid[:,2],self.int_grid[:,1],self.int_grid[:,0],:] = disp[:,:].float()
...
def deform_volume(V,ess_grid,out_grid,ind_ess_grid,box_size,vol_type = 'volume'):
    ...
    for i in range(batch_size):
        if vol_type == 'volume':
            ...
            Vnew[i,ess_grid_int[:,0],ess_grid_int[:,1],ess_grid_int[:,2]] += V[i,ind_ess_grid[:,0],ind_ess_grid[:,1],ind_ess_grid[:,2]] #substitute deformed values
            Vnew[i,out_grid_int[:,0],out_grid_int[:,1],out_grid_int[:,2]] += V[i,out_grid_int[:,0],out_grid_int[:,1],out_grid_int[:,2]]
        elif vol_type == 'ctf':
            Vnew[i,ess_grid_int[:,0],ess_grid_int[:,1],ess_grid_int[:,2]] += V[i,ind_ess_grid[:,0],ind_ess_grid[:,1],ind_ess_grid[:,2]]
            Vnew[i,out_grid_int[:,0],out_grid_int[:,1],out_grid_int[:,2]] += V[i,out_grid_int[:,0],out_grid_int[:,1],out_grid_int[:,2]]
...
def get_grad(ess_grid,ind_ess_grid,box_size):
    ...
    det = gx[0]*gy[1]*gz[2]-gx[0]*gy[2]*gz[1]-gx[1]*gy[0]*gz[2]+gx[1]*gy[2]*gz[0]+gx[2]*gy[0]*gz[1]-gx[2]*gy[1]*gz[0]
        ...
        for dx in (0,1):
            ...
            for dy in (0,1):
                ...
                for dz in (0,1):
                    ...
                    valid = ((0<=x_)*(x_ < box_size)*(0<=y_)*(y_< box_size)*(0<=z_)*(z_<box_size)).long()
                    w = (w*valid.type_as(w))
                    values = V[i,x_,y_,z_]
                    vol += w*values
...
class rotate_volume(nn.Module):
    ...
    def forward(self, V,rr):
        ...
        if R.dim()==2:
            R = R.unsqueeze(0)
        G = torch.nn.functional.affine_grid(R, (batch_size,1,self.box_size+1,self.box_size+1,self.box_size+1), align_corners=False).float()
...
def generate_filter(box_size, filter_type = 'ramlak', dimension = 2):
    ...
    if dimension ==2:
        ...
        R = torch.fft.fftshift(torch.pow(GG[0]**2+GG[1]**2+GG[2]**2,0.5),dim=[-1,-2,-3]).to(device)
        mask = R>box_size/2
        if filter_type == 'ramlak':
            filt = R
        elif filter_type == 'cosine':
            filt = torch.fft.fftshift(R*torch.cos(np.pi*R/box_size),dim=[-1,-2,-3]).to(device)
        elif filter_type == 'shepplogan':
...
inv_half1 = inverse_displacement(device, latent_dim, N_points, 6, 96, lin_block, 6 ,box_size).to(device)
inv_half2 = inverse_displacement(device, latent_dim, N_points, 6, 96, lin_block, 6, box_size).to(device)
    ...
    for batch_ndx, sample in enumerate(data_loader_half1):
        ...
        if epochs >0 and args.noise == 'False':
            ...
            with torch.no_grad():
                ...
                ctfs_l = torch.nn.functional.pad(ctf,(box_size//2,box_size//2,box_size//2,box_size//2,0,0)).to(device)
                ...
                if args.noise == 'True':
                    noise_real = (5/(ang_pix*box_size))*torch.randn_like(cons_model.pos).to(device)
                    ...
                    proj,proj_im,proj_pos,pos,dis = decoder_half1.forward(mu_in,r.to(device),evalpos.to(device),torch.ones_like(evalpos[:,0]),torch.ones(n_classes,evalpos.shape[0]).to(device),t.to(device))
                else:
                    proj,proj_im,proj_pos,pos,dis = decoder_half1.forward(mu_in,r.to(device),cons_model.pos.to(device),cons_model.amp.to(device),cons_model.ampvar.to(device),t.to(device))
        ...
        if epochs>0 and args.noise == 'False':
            ...
            with torch.no_grad():
                ...
                if args.noise == 'True':
                    ...
                    proj,proj_im,proj_pos,pos,dis = decoder_half2.forward(mu_in,r.to(device),cons_model.pos.to(device),cons_model.amp.to(device),cons_model.ampvar.to(device),t.to(device))
                    mu_in_tot[idx] = mu.cpu()
                    pos_tot[idx] = pos.cpu()
...
checkpoint = {'inv_half1': inv_half1, 'inv_half2': inv_half2, 'inv_half1_state_dict': inv_half1.state_dict(), 'inv_half2_state_dict': inv_half2.state_dict()}
...
id_grid = torch.nn.functional.affine_grid(id_trans,(1,1,box_size,box_size,box_size),align_corners=True)
...
if args.random_half:
    ...
    for j in rel_inds:
         ...
         if len(tile_indices) >args.ignore:
             with torch.no_grad():
                 _,_,_,n_pos,deformation = decoder_half1(z_tile,r.to(device),supersmallgrid.to(device),cons_model.amp.to(device),torch.ones(n_classes,supersmallgrid.shape[0]).to(device),t.to(device))
                 ...
                 if i%100==0:
                     ...
                     field2bild(n_pos[0],im_deformation[0],'/cephfs/schwab/fields_mon/Ninversefield'+str(i).zfill(3),box_size,ang_pix)
                     field2bild(supersmallgrid.to(device),n_pos[0],'/cephfs/schwab/fields_mon/Nforwardfield'+str(i).zfill(3),box_size,ang_pix)
                     field2bild(supersmallgrid.to(device),im_deformation[0],'/cephfs/schwab/fields_mon/Nfwdbwdfield'+str(i).zfill(3),box_size,ang_pix)
                     print('saved field')

                 
                 ind_ess_grid = torch.clip(torch.round((tile_deformation[0]+0.5)*(box_size-1)),min = 0, max = box_size-1).long()
                 in_grid = torch.clip((ess_grid+0.5)*(box_size-1),min = 0, max = box_size-1).long()
                 ...
                 print('Backprojected', bp_imgs.cpu().numpy(), 'particles from', len(dataset), 'and omitted backprojection of', om_imgs.cpu().numpy(), 'particles')
         ...
         if len(tile_indices) >args.ignore:
             print('Starting backprojection of tile', j ,'from', gxy.shape[0],'::', len(tile_indices), 'images')
             i = i+1
             if i %50 ==0:
                 try:
                     VV = V[:,:,:]*sm_bin_mask
                 ...
                 if args.filter == 'ctf':
                     VV2 = torch.real(torch.fft.ifftn(VV/torch.maximum(CTF,lam_thres*torch.ones_like(CTF)),dim = [-1,-2,-3]))

                 with mrcfile.new('/cephfs/schwab/backprojection/volumes14/reconstruction_half1_' + str(i//50).zfill(3) + '.mrc', overwrite=True) as mrc:
             ...
             with torch.no_grad():
                 for batch_ndx, sample in enumerate(tqdm(current_data_loader)):
                     ...
                     y_in = data_preprocessor.apply_translation(y_in, -t[:, 0], -t[:, 1])
                     y_in = data_preprocessor.apply_circular_mask(y_in)
                     y_in,r,t,ctfs,ctf = y_in.to(device), r.to(device),t.to(device), ctfs.to(device),ctf.to(device)
                     if args.ctf:
                         ...
                         yr = y_in.unsqueeze(1)
                     if args.filter == 'ctf':
                         ctfr2_l = torch.fft.fftshift(torch.real(torch.fft.ifft2(torch.fft.fftshift(ctfs_l,dim = [-1,-2]),dim = [-1,-2],)).unsqueeze(1),dim = [-1,-2])
                         ctfr2_lc = torch.nn.functional.avg_pool2d(ctfr2_l, kernel_size=3, stride=2, padding = 1)
                         ...
                         CTFy = CTFy[:,:-1,:-1,:-1]
                         CTF += (1/len(dataset))*torch.real(torch.sum(torch.fft.fftn(CTFy.unsqueeze(1),dim = [-1,-2,-3])**2,0).squeeze())
                     ...
                     Vy = torch.sum(Vy,0)
                     Vy = F.grid_sample(input=Vy.unsqueeze(0).unsqueeze(0), grid=dis_grid.to(device), mode='bilinear',  align_corners=False)
    ...
    if args.filter == 'ctf':
        V = torch.real(torch.fft.ifftn(V/torch.maximum(CTF,lam_thres*torch.ones_like(CTF)),dim = [-1,-2,-3]))
    ...
    for j in rel_inds:
         ...
         if len(tile_indices) >args.ignore:
             ...
             if i %50 ==0:
                 ...
                 with mrcfile.new('/cephfs/schwab/backprojection/volumes14/reconstruction_half2_' + str(i//50).zfill(3) + '.mrc', overwrite=True) as mrc:
                     mrc.set_data((VV2/torch.max(VV2)).float().cpu().numpy())
