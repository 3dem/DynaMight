...
from dynamight.utils.utils_new import initialize_dataset, add_weight_decay, field2bild
from dynamight.models.decoder import InverseDisplacementDecoder
from dynamight.models.blocks import LinearBlock
from dynamight.deformable_backprojection.bp_utils import get_ess_grid, DeformationInterpolator, RotateVolume, generate_mask
...
def deformable_backprojection(
    ...
    cons_model.ampvar = torch.nn.Parameter(torch.ones(n_classes,decoder_half1.n_points).to(device),requires_grad = False)
    ...
    dataset,diameter_ang,box_size,ang_pix,optics_group = initialize_dataset(refinement_star_file, circular_mask_thickness,preload_images,particle_diameter)
    ...
    ess_grid, out_grid, sm_bin_mask = generate_mask(mask_file,device)
    ...
    fwd_int = DeformationInterpolator(device, smallgrid, smallgrid, box_size, downsample)
            ...
            with torch.no_grad():
                ...
                if i % 200 == 0:
                    ...
                    field2bild(n_pos[0], im_deformation[0],
                               output_directory+'Ninversefield' + str(i).zfill(3),
                               box_size, ang_pix)
                    field2bild(supersmallgrid.to(device), n_pos[0],
                               output_directory+'Nforwardfield' + str(i).zfill(3),
                               box_size, ang_pix)
                    field2bild(supersmallgrid.to(device), im_deformation[0],
                               output_directory+'Nfwdbwdfield' + str(i).zfill(3), box_size,
                               ang_pix)
                    print('saved field')
...
def learn_inverse_deformation_field(
    ...
    cp = torch.load(vae_directory,map_location=device)
    ...
    inv_half1 = InverseDisplacementDecoder(device, latent_dim, N_points, 6, 96, LinearBlock, 6 ,box_size).to(device)
    inv_half2 = InverseDisplacementDecoder(device, latent_dim, N_points, 6, 96, LinearBlock, 6, box_size).to(device)
