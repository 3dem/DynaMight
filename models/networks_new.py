...
class gaussian_decoder(torch.nn.Module):
    def __init__(self,box_size,device,kernel,latent_dim,n_points,n_layers,n_neurons,block):
        ...
        self.pos = torch.nn.Parameter(0.01*(torch.rand(n_points,3)-self.ini),requires_grad=True)
    ...
    def forward(self,z,r,d):
        ...
        # else:
        Proj_im = self.p2i(Proj_pos,self.amp*torch.stack(self.batch_size*[torch.ones(self.n_points)]).to(self.device))
        Proj = Proj_im
...
class multi_gaussian_decoder(torch.nn.Module):
    def __init__(self,box_size,device,kernel,latent_dim,n_points,n_classes,n_layers,n_neurons,block):
    ...
    def forward(self,z,r,d):
        ...
        Proj_im = self.p2i(Proj_pos,torch.stack(self.batch_size*[self.amp*torch.nn.functional.softmax(self.ampvar,dim = 0)],dim=0).to(self.device))
    ...
    def volume(self,z,r,defo=1):
        ...
        V = self.p2v(pos,torch.stack(bs*[torch.nn.functional.softmax(self.ampvar,dim=0)],0)*self.amp.to(self.device))
    ...
    def add_points(self,th,ang_pix):
        ...
        self.pos = torch.nn.Parameter(torch.cat([self.pos,np],0),requires_grad=True)
        self.ampvar = torch.nn.Parameter(torch.cat([self.ampvar,torch.rand_like(self.ampvar[:,probs[fatclass,:]>th])],1),requires_grad=True)
...
class gaussian_decoder_het(torch.nn.Module): #Decoder with the possibility to add and remove density
    def __init__(self,box_size,device,kernel,latent_dim,n_points,n_layers,n_neurons,block):
        ...
        self.ini = .5*torch.ones(3)
        self.ini[2] = 1
        self.pos = torch.nn.Parameter(0.1*(torch.rand(n_points,3)-self.ini),requires_grad=True)
        #self.pos = torch.nn.Parameter(0.03*(torch.rand(n_points,3)-0.5),requires_grad=True)
        self.act = torch.nn.Tanh()
        self.amp = torch.nn.Parameter(0.1*torch.ones(1),requires_grad = True)
        self.ampvar = torch.nn.Parameter(torch.rand(n_points),requires_grad=True)
    ...
    def forward(self,z,r,d):
        ...
        Proj_pos = self.proj(pos,r)
        Proj_im = self.p2i(Proj_pos,self.amp_corr*self.amp*torch.stack(self.batch_size*[torch.ones(self.n_points)]).to(self.device))
        Proj = Proj_im
        return Proj,Proj_im, Proj_pos, pos, res
...
class het_encoder(torch.nn.Module):
    ...
    def __init__(self,box_size,latent_dim,down_sample):
        ...
        self.down = nn.AvgPool2d(2,stride = 2)
        self.lin1 = nn.Linear(in_features=int((box_size/(2**down_sample))**2),out_features=256)
        #self.lin1a = nn.Linear(in_features = 256, out_features = 256)
...
class gaussian_decoder_het2(torch.nn.Module): # coordinate based density value (needs extra latent dimension for density)
    def __init__(self,box_size,device,kernel,latent_dim,n_points,n_layers,n_neurons,block,diff_conf = False):
...
class implicit_pointnet(torch.nn.Module):
    ...
    def forward(self,z,r,d):
        ...
        V_high = self.p2v_high(pos,self.amp*torch.stack(self.batch_size*[torch.ones_like(self.x)],0))
...
class consensus_model(torch.nn.Module):
    def __init__(self,box_size,device,n_points,n_classes,oversampling = 1):
        ...
        self.n_points = n_points
        self.ini = .5*torch.ones(3)
        self.pos = torch.nn.Parameter(0.035*(torch.rand(n_points,3)-self.ini),requires_grad=True)
    ...
    def forward(self,r,shift):
        ...
        Proj_im = self.p2i(Proj_pos,torch.stack(self.batch_size*[torch.clip(self.amp,min = 1)*torch.nn.functional.softmax(self.ampvar,dim = 0)],dim=0).to(self.device))
        Proj = self.i2F(Proj_im)
    ...
    def volume(self,r,shift):
        ...
        V = self.p2v(pos,torch.stack(bs*[torch.nn.functional.softmax(self.ampvar,dim = 0)],0)*torch.clip(self.amp,min = 1).to(self.device))
        #V = self.p2v(pos,torch.stack(bs*[self.ampvar],0)*self.amp.to(self.device))
    ...
    def double_points(self,ang_pix,dist):
        ...
        eps = (dist/(self.box_size*ang_pix))*torch.stack([torch.cos(theta)*torch.sin(phi),torch.sin(theta)*torch.sin(phi),torch.cos(phi)],1)
        np = self.pos+eps
        self.pos = torch.nn.Parameter(torch.cat([self.pos,np],0),requires_grad=True)
        self.amp = torch.nn.Parameter(0.8*self.amp,requires_grad = True)
        self.ampvar = torch.nn.Parameter(torch.cat([self.ampvar,torch.rand_like(self.ampvar)],1),requires_grad=True)
        self.n_points = self.pos.shape[0]
...
class displacement_decoder(torch.nn.Module):
    ...
    def volume(self,z,r,cons,amp, ampvar,shift):
        ...
        V = self.p2v(pos,torch.stack(bs*[torch.nn.functional.softmax(ampvar,dim = 0)],0)*torch.clip(amp,min = 1).to(self.device))
...
class inverse_displacement(torch.nn.Module):
    def __init__(self,device,latent_dim,n_points,n_layers,n_neurons,block,pos_enc_dim,box_size, mask = None):
...
class displacement_decoder_amp(torch.nn.Module):
    ...
    def forward(self,z,r,cons,amp, ampvar,shift,z_amp = None):
        ...
        Proj_im = self.p2i(Proj_pos,amp_corr*torch.stack(self.batch_size*[amp*torch.nn.functional.softmax(ampvar,dim = 0)],dim=0).to(self.device))
        Proj = self.i2F(Proj_im)
        Proj = fourier_shift_2d(Proj.squeeze(),shift[:,0],shift[:,1])
    ...
    def volume(self,z,r,cons,amp, ampvar,shift,z_amp):
        ...
        V = self.p2v(pos,amp_corr*torch.stack(bs*[torch.nn.functional.softmax(ampvar,dim=0)],0)*amp.to(self.device))
        V = torch.fft.fftn(V,dim = [-3,-2,-1])
        R, M = radial_index_mask3(self.box_size)
        R = torch.stack(self.i2F.n_classes*[R.to(self.device)],0)
        FF = torch.exp(-self.i2F.B[:,None,None,None]**2*R)*self.i2F.A[:,None,None,None]**2
...
class image_decoder(torch.nn.Module):
    ...
    def forward(self, z, r, y):
        ...
        out = torch.nn.functional.grid_sample(y.unsqeeze(1),def_field,mode = 'bilinear',align_corners=False)
        return out
