...
parser.add_argument('--n_residues', type=int, default=None,
                    help='Estimated number of residues to choose the number of gaussians')
parser.add_argument('--n_gauss', type=int, default=20000, help='Number of gaussian basis functions')
parser.add_argument('--n_layers', type=int, default=5,
                    help='Number of linear layers for the deformation model')
...
parser.add_argument('--n_widths',
                    help='number of different gaussians (for determining location of sparse areas of lower resolution)',
                    type=int, default=1)
parser.add_argument('--pos_enc_dim', help='dimension parameter for positional encoding', type=int,
                    default=10)
parser.add_argument('--mask', help='directory of mask file (needs changes in the file)', type=str)
parser.add_argument('--substitute',
                    help='update consensus model by taking a step of subsitute value in direction of closest decoder output',
...
parser.add_argument('--regularization',
                    help='weighting between regularization term and data term (1 = equal weighting, 0 = no regularization), more rigidity for high value',
                    type=float, default=0.2)
parser.add_argument('--add_free_gaussians', help='number of unconstrained gaussians', type=int,
                    default=0)
...
dataset, diameter_ang, box_size, ang_pix, optics_group = initialize_dataset(args.input,
...
if args.random_half:
    if args.checkpoint:
        ...
        dataset_half1, dataset_half2 = torch.utils.data.dataset.random_split(dataset,
                                                                             [len(dataset) // 2,
                                                                              len(dataset) - len(
                                                                                  dataset) // 2])
    ...
    print('Initialized data loaders for half sets of size', len(dataset_half1), ' and ',
          len(dataset_half2))
...
print('box size:', box_size, 'pixel_size:', ang_pix, 'virtual pixel_size:', 1 / (box_size + 1),
      ' dimension of latent space: ', args.latent_dim)
...
if args.model:
    mode = 'model'
    cons_model, gr = optimize_coarsegraining(args.model, box_size, ang_pix, device, args.log_dir,
                                             args.n_widths, args.add_free_gaussians, resolution=8)
...
try:
    cons_model.i2F.A = torch.nn.Parameter(torch.linspace(0.01, 0.02, n_classes).to(device),
                                          requires_grad=False)
...
if args.random_half:
    deformation_half1 = displacement_decoder(box_size, device, latent_dim, N_points, n_classes,
                                             n_layers, n_neurons,
                                             lin_block, pos_enc_dim, 3).to(device)
    deformation_half2 = displacement_decoder(box_size, device, latent_dim, N_points, n_classes,
                                             n_layers, n_neurons,
                                             lin_block, pos_enc_dim, 3).to(device)
    encoder_half1 = het_encoder(box_size, latent_dim, 1).to(device)
    encoder_half2 = het_encoder(box_size, latent_dim, 1).to(device)
else:
    deformation = displacement_decoder(box_size, device, latent_dim, N_points, n_classes, n_layers,
                                       n_neurons,
...
if args.checkpoint:
    encoder_half1, encoder_half2, deformation_half1, deformation_half2 = load_models(
        args.checkpoint, device, box_size, n_classes)
...
with torch.no_grad():
    if args.model:
        ...
        gr1 = radius_graph(cons_model.pos * ang_pix * box_size, distance + distance / 2,
                           num_workers=8)
        gr2 = knn_graph(cons_model.pos, 1, num_workers=8)
        cons_dis = torch.pow(torch.sum((cons_positions[gr1[0]] - cons_positions[gr1[1]]) ** 2, 1),
                             0.5)
...
if args.random_half:
    ...
    for epoch in range(N_epochs):
        ...
        if epoch == consensus and args.model != None:
            deformation_half1.i2F.A = torch.nn.Parameter(cons_model.i2F.A.to(device),
                                                         requires_grad=True)
            deformation_half1.i2F.B = torch.nn.Parameter(cons_model.i2F.B.to(device),
                                                         requires_grad=True)
            deformation_half2.i2F.A = torch.nn.Parameter(cons_model.i2F.A.to(device),
                                                         requires_grad=True)
            deformation_half2.i2F.B = torch.nn.Parameter(cons_model.i2F.B.to(device),
                                                         requires_grad=True)
            dec_half1_params = deformation_half1.parameters()
            dec_half2_params = deformation_half2.parameters()
        ...
        if args.calibrate_loss_weights:
            calibration_data = torch.utils.data.Subset(dataset,
                                                       torch.randint(0, len(dataset), (1000,)))
            data_loader_cal = DataLoader(
                dataset=calibration_data,
                batch_size=batch_size,
                num_workers=8,
            ...
            for batch_ndx, sample in enumerate(data_loader_cal):
                ...
                r, y, ctf, shift = sample["rotation"], sample["image"], sample["ctf"], -sample[
                    'translation']
                idx = sample['idx']
                ...
                y_in = data_preprocessor.apply_translation(y_in.detach(), -shift[:, 0].detach(),
                                                           -shift[:, 1].detach())
        ...
        for batch_ndx, sample in enumerate(data_loader_half1):
            ...
            if epoch < consensus and learn_consensus == True:  # Set latent code for consensus reconstruction to zero
                ...
                with torch.no_grad():
                    ...
                    z_in2 = [mu2]
                    _, _, _, _, d_points2 = deformation_half2(z_in2, r, cons_model.pos.to(device),
                                                              cons_model.amp.to(device),
                                                              cons_model.ampvar.to(device),
                                                              shift.to(device))
            ...
            latent_loss = -0.5 * torch.mean(
                torch.sum(1 + logsigma - mu ** 2 - torch.exp(logsigma), dim=1), dim=0)
        ...
        for batch_ndx, sample in enumerate(data_loader_half2):
            ...
            if epoch < consensus and learn_consensus == True:  # Set latent code for consensus reconstruction to zero
                ...
                Proj, P, PP, n_points, d_points = deformation_half2(z_in, r,
                                                                    cons_model.pos.to(device),
                                                                    cons_model.amp.to(device),
                                                                    cons_model.ampvar.to(device),
                                                                    shift.to(device))
                ...
                with torch.no_grad():
                    ...
                    z_in2 = [mu2]
                    _, _, _, _, d_points2 = deformation_half1(z_in2, r, cons_model.pos.to(device),
                                                              cons_model.amp.to(device),
                                                              cons_model.ampvar.to(device),
                                                              shift.to(device))
                    mean_positions += torch.sum(n_points.detach(), 0)
        ...
        poses = pose_model(box_size, device, torch.tensor(current_angles),
                           torch.tensor(current_shifts))
        try:
            gr_old = gr1
        except:
        ...
        if epoch > (consensus - 1) and args.substitute != 0:
            ...
            if (
                running_recloss_half1 < old_loss_half1 and running_recloss_half2 < old_loss_half2) and args.substitute != 0:
                cons_model.pos = torch.nn.Parameter(
                    (1 - args.substitute) * cons_model.pos + args.substitute * min_npos,
                    requires_grad=False)
        ...
        with torch.no_grad():
            ...
            if tot_latent_dim > 2:
                if epoch % 5 == 0 and epoch > consensus:
                    summ.add_figure(f"Data/latent",
                                    visualize_latent(latent_space, c=diff / (np.max(diff) + 1e-7),
                                                     s=3, alpha=0.2, method='pca'),
                                    epoch)
                ...
                summ.add_figure(f"Data/latent",
                                visualize_latent(latent_space, c=diff / (np.max(diff) + 1e-22), s=3,
                                                 alpha=0.2),
                                epoch)
                summ.add_figure(f"Data/latent2",
                                visualize_latent(latent_space, c=cols, s=3, alpha=0.2),
                                epoch)
            summ.add_scalar(f"Loss/kld_loss", (running_latloss_half1 + running_latloss_half2) / (
                    len(data_loader_half1) + len(data_loader_half2)), epoch)
            summ.add_scalar(f"Loss/mse_loss", (running_recloss_half1 + running_recloss_half2) / (
                    len(data_loader_half1) + len(data_loader_half2)), epoch)
            summ.add_scalars(f"Loss/mse_loss_halfs",
                             {'half1': (running_recloss_half1) / (len(data_loader_half1)),
                              'half2': (running_recloss_half2) / (len(data_loader_half2))}, epoch)
            summ.add_scalar(f"Loss/total_loss",
                            (running_total_loss_half1 + running_total_loss_half2) / (
                                    len(data_loader_half1) + len(data_loader_half2)), epoch)
            summ.add_scalar(f"Loss/dist_loss", (var_total_loss_half1 + var_total_loss_half2) / (
                    len(data_loader_half1) + len(data_loader_half2)), epoch)
            if epoch < consensus:
                summ.add_scalar(f"Loss/variance", cons_model.i2F.B[0].detach().cpu(), epoch)
            else:
                summ.add_scalar(f"Loss/variance1a", deformation_half1.i2F.B[0].detach().cpu(),
                                epoch)
                summ.add_scalar(f"Loss/variance2a", deformation_half2.i2F.B[0].detach().cpu(),
                                epoch)
            ...
            summ.add_figure(f"Data/output", tensor_imshow(
                torch.fft.fftshift(apply_CTF(Proj[0], ctf[0].float()).squeeze().cpu(),
                                   dim=[-1, -2])), epoch)
            summ.add_figure(f"Data/input", tensor_imshow(y_in[0].squeeze().detach().cpu()), epoch)
            summ.add_figure(f"Data/target", tensor_imshow(
                torch.fft.fftshift(apply_CTF(y[0], BF.float()).squeeze().cpu(), dim=[-1, -2])),
                            epoch)
            summ.add_figure(f"Data/cons_points_z_half1",
                            tensor_scatter(cons_model.pos[:, 0], cons_model.pos[:, 1],
                                           c=mean_dist_half1, s=3), epoch)
            summ.add_figure(f"Data/cons_points_z_half2",
                            tensor_scatter(cons_model.pos[:, 0], cons_model.pos[:, 1],
                                           c=mean_dist_half2, s=3), epoch)
            summ.add_figure(f"Data/delta",
                            tensor_scatter(n_points[0, :, 0], n_points[0, :, 1], 'b', s=0.1), epoch)
            summ.add_figure(f"Data/delta_h1",
                            tensor_scatter(d_points2[0, :, 0], d_points2[0, :, 1], 'b', s=0.1),
                            epoch)
            ...
            summ.add_figure(f"Data/projection_image",
                            tensor_imshow(torch.fft.fftshift(torch.real(
                                torch.fft.ifftn(Proj[0], dim=[-1, -2])).squeeze().detach().cpu(),
                                                             dim=[-1, -2])), epoch)
            summ.add_figure(f"Data/mod_model", tensor_imshow(
                apply_CTF(apply_CTF(y[0], ctf[0].float()),
            ...
            summ.add_figure(f"Data/avg_err",
                            tensor_imshow(torch.fft.fftshift(err_avg, dim=[-1, -2]).cpu()), epoch)
        ...
        if epoch % 1 == 0:
            with torch.no_grad():
                ...
                points2xyz(cons_model.pos, log_dir + '/positions' + str(epoch).zfill(3), box_size,
                           ang_pix, types)
                points2xyz(mean_positions, log_dir + '/mean_positions' + str(epoch).zfill(3),
                           box_size, ang_pix, types)
                ...
                with mrcfile.new(log_dir + '/volume' + str(epoch).zfill(3) + '.mrc',
    ...
    for epoch in range(N_epochs):
        if epoch > consensus and (
            epoch + consensus) % 5 == 0 and cons_model.n_points < args.n_gauss:
            cons_model.double_points(ang_pix, mean_neighbour_dist)
            cons_params = cons_model.parameters()
            cons_optimizer = torch.optim.Adam(cons_params, lr=posLR)
        ...
        for batch_ndx, sample in enumerate(data_loader):
            ...
            imizer.zero_grad()
            # TODO: remove all this or make the optimizer somewhere
            ...
            if epoch < consensus and learn_consensus == True:  # Set latent code for consensus reconstruction to zero
                ...
                Proj, P, PP, n_points, d_points = deformation(z_in, r, cons_model.pos.to(device),
                                                              cons_model.amp.to(device),
                                                              cons_model.ampvar.to(device),
                                                              shift.to(device))
            ...
            if cons_model.n_points < args.n_gauss // 9:
                ...
                geo_loss = geo_loss = geometric_loss(n_points, box_size, ang_pix, distance,
                                                     deformation=cons_dis, graph1=gr1, graph2=gr2,
                                                     neighbour=True, distance=False, outlier=False)
                # geo_loss = torch.zeros(1).to(device)
            else:
                encoder.requires_grad = True
                # var_loss = batch_structure(gr1,gr2,n_points,box_size,ang_pix,device,distance)
                ...
                geo_loss = geometric_loss(n_points, box_size, ang_pix, distance,
                                          deformation=cons_dis, graph1=gr1, graph2=gr2,
                                          neighbour=True, distance=True, outlier=False)
                # geo_loss = torch.zeros(1).to(device)

            loss = rec_loss + beta * kld_weight * latent_loss + 0.0005 * np.floor(
                epoch / 100) * geo_loss
        ...
        if epoch > consensus and args.substitute != 0:
            ...
            with torch.no_grad():
                for batch_ndx, sample in enumerate(random_sub_loader):
                    ...
                    z_in = [z]
                    # z_amp = z[:,-1:]
                    Proj, P, PP, n_points, d_points = deformation(z_in, r,
                                                                  cons_model.pos.to(device),
                                                                  cons_model.amp.to(device),
                                                                  cons_model.ampvar.to(device),
                                                                  shift.to(device))
                ...
                if running_recloss < old_loss and args.substitute != 0:
                    new_pos = min_npos
                    cons_model.pos = torch.nn.Parameter(cons_model.pos + args.substitute * min_dpos,
                                                        requires_grad=True)
        ...
        with torch.no_grad():
            ...
            if tot_latent_dim > 2:
                if epoch % 5 == 0 and epoch > consensus:
                    summ.add_figure(f"Data/latent", visualize_latent(latent_space,
                                                                     c=np.linspace(0, 1,
                                                                                   latent_space.shape[
                                                                                       0]), s=3,
                                                                     alpha=0.05, method='pca'),
                                    epoch)
            ...
            summ.add_scalar(f"Loss/pose_error", angular_error, epoch)
            summ.add_scalar(f"Loss/trans_error", translational_error, epoch)
            summ.add_figure(f"Data/output",
                            tensor_imshow(apply_CTF(P[0], ctf[0].float()).squeeze().detach().cpu()),
                            epoch)
            summ.add_figure(f"Data/input", tensor_imshow(y_in[0].squeeze().detach().cpu()), epoch)
            summ.add_figure(f"Data/cons_points_z",
                            tensor_scatter(cons_model.pos[:, 0], cons_model.pos[:, 1], c=mean_dist,
                                           s=3), epoch)
            summ.add_figure(f"Data/delta",
                            tensor_scatter(n_points[0, :, 0], n_points[0, :, 1], 'b', s=0.1), epoch)
            # summ.add_figure(f"Data/min_n_pos", tensor_scatter(min_npos[:, 0], min_npos[:, 1], 'b', s=0.1), epoch)
            ...
            summ.add_figure(f"Data/projection_image",
                            tensor_imshow(apply_CTF(P[0], torch.ones_like(
                                ctf[0])).float().squeeze().detach().cpu()), epoch)
            summ.add_figure(f"Data/shapes", tensor_plot(FF), epoch)
