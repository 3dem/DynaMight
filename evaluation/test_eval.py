...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 28 10:17:42 2022
...
# -*- coding: utf-8 -*-
"""
Created on Mon Jul 26 09:44:20 2021

@author: schwab
"""

# File to visualize 2D latent space and manually pick points A and B by looking at the image to get the volumes along the trajectory
# on the segment [A,B]
...
from voxelium.base.grid import idht
...
parser.add_argument('checkpoint', help='path to load the networks checkpoint', type=str)
parser.add_argument('particle_dir', type=str, metavar='log_dir', help='input job (job directory or optimizer-file)')
parser.add_argument('--mask', help='directory of mask file', type= str)
parser.add_argument('--preload', action='store_true')
parser.add_argument('--gpu', help='which gpu', type = str, default = '-1')
parser.add_argument('--batch_size', type = int, default = 100)
parser.add_argument('--half', type = int, default = 1)
parser.add_argument('--out_dir', type = str, help = 'directory to save new star files')
parser.add_argument('--pdb_series', type = str, help = 'save pdb series')
...
dataset,diameter_ang,box_size,ang_pix,optics_group = initialize_dataset(args.particle_dir, args.circular_mask_thickness,args.preload,args.particle_diameter)
#dataset = RelionDataset('/cephfs/dari/georges_6cAF55DNA-krios/Refine3D/job024').make_particle_dataset()
#dataset = RelionDataset('/cephfs/dari/waving_spike/waving_spike/refine3d').make_particle_dataset()
#dataset = RelionDataset('/cephfs/dari/plasmodium/Refine3D/job006_3A').make_particle_dataset()
#optics_groups = dataset.get_optics_group_stats()
#if len(optics_groups) > 1:
#    raise RuntimeError("Only a single optics groups is supported.")
...
decoder.load_state_dict(cp['decoder_'+'half'+str(args.half)+'_state_dict'])
...
with torch.no_grad():
    for batch_ndx, sample in enumerate(data_loader_half1):
        ...
        mu, _ = encoder(y,ctf)
        mu_in = [mu]
        _,_,_,_,dis = decoder(mu_in,r,cons_model.pos.to(device),cons_model.amp.to(device),cons_model.ampvar.to(device),t)
        ...
        if batch_ndx == 0:
            ...
            c4 = torch.sum(cons.movedim(2,0)*((d>2*pixel_distance)*d),2)/dps[None,:]

        else:
            z = torch.cat([z,mu])
            c1 = torch.cat([c1,torch.sum(d>pixel_distance,1)])
            c2 = torch.cat([c2,torch.sum(d,1)])
            c3 = torch.cat([c3,torch.mean(dis.movedim(2,0)*(d>pixel_distance),-1)],1)
            c4 = torch.cat([c4,torch.sum(cons.movedim(2,0)*((d>2*pixel_distance)*d),2)/dps[None:]],1)
...
nap_cons_pos = torch.stack([nap_cons_pos[:,0],nap_cons_pos[:,3],nap_cons_pos[:,2],nap_cons_pos[:,1]],1)
...
lat_colors = {'amount': cc1,'direction': cc3, 'location': cc5, 'index': indices, 'pose': ccp, 'shift': cct }

class visualizer:
    def __init__(self, z,latent, decoder,cons_model,V0,cons_volume,nap_cons_pos,dd,amps,lat_cols,starfile,indices,latent_closest):
        self.z = z
        self.latent = latent
        ...
        self.vol_layer = self.vol_viewer.add_image(self.V0,rendering = 'iso',colormap = 'gray', iso_threshold = 0.15)
        self.cons_layer = self.vol_viewer.add_image(self.cons_volume,rendering = 'iso',colormap = 'gray', iso_threshold = 0.15)
        self.point_properties = {'activity': dd,'amplitude': amps, 'width': torch.nn.functional.softmax(cons_model.ampvar,0)[0].detach().cpu()}
        self.nap_cons_pos = nap_cons_pos
        self.nap_zeros = torch.zeros(nap_cons_pos.shape[0])
        self.point_layer = self.vol_viewer.add_points(self.nap_cons_pos, properties = self.point_properties,ndim = 4, size = 1.5,face_color = 'activity',face_colormap = 'jet', edge_width = 0,visible = False)
        self.cons_point_layer = self.vol_viewer.add_points(self.nap_cons_pos, properties = self.point_properties,ndim = 4, size = 1.5,face_color = 'activity',face_colormap = 'jet', edge_width = 0,visible = False)
        ...
        self.lat_im = self.vol_viewer.window.add_dock_widget(self.lat_canv,area = 'right', name = 'latent space')
        self.vol_viewer.window._qt_window.resizeDocks([self.lat_im],[800],Qt.Horizontal)
        self.rep_menu = widgets.ComboBox(label = '3d representation', choices = ['volume','points'])
        self.rep_widget = widgets.Container(widgets=[self.rep_menu])
        self.repw = self.vol_viewer.window.add_dock_widget(self.rep_widget,area = 'bottom', name = '3d representation')
        self.rep_col_menu = widgets.ComboBox(label = 'gaussian colors', choices = ['displacement','amplitude','width'])
        self.rep_col_widget = widgets.Container(widgets=[self.rep_col_menu])
        self.repc = self.vol_viewer.window.add_dock_widget(self.rep_col_widget,area = 'bottom', name = 'gaussian colors')
        self.rep_action_menu = widgets.ComboBox(label = 'action', choices = ['click','trajectory','particle number','starfile'])
        self.rep_action_widget = widgets.Container(widgets=[self.rep_action_menu])
        self.repa = self.vol_viewer.window.add_dock_widget(self.rep_action_widget,area = 'bottom', name = 'action')
        self.color_menu = widgets.ComboBox(label = 'color by', choices = ['direction', 'amount','location','density','log-density','index','pose','shift'])
        self.color_widget = widgets.Container(widgets=[self.color_menu])
        self.colw = self.vol_viewer.window.add_dock_widget(self.color_widget,area = 'right', name = 'colors')
        ...
        self.poly = PolygonSelector(ax = self.axes1, onselect = self.get_part_nr, props = self.line,useblit = True)
    ...
    def coloring_changed(self,event):
            ...
            if selected_label == 'amount':
                self.axes1.clear()
                self.axes1.scatter(self.z[:,0],self.z[:,1],c = self.lat_cols['amount'],alpha = 0.1)
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()

            elif selected_label == 'direction':
                self.axes1.clear()
                self.axes1.scatter(self.z[:,0],self.z[:,1],c = self.lat_cols['direction'],alpha = 0.1)
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()
            
            elif selected_label == 'location':
                self.axes1.clear()
                self.axes1.scatter(self.z[:,0],self.z[:,1],c = self.lat_cols['location'],alpha = 0.1)
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()
            elif selected_label == 'density':
                h, x, y, p = plt.hist2d(self.z[:,0],self.z[:,1],bins = (50,50), cmap = 'hot')
                ex = [self.axes1.dataLim.x0,self.axes1.dataLim.x1,self.axes1.dataLim.y0,self.axes1.dataLim.y1]
                self.axes1.clear()
                #axes1.hist2d(zz[:,0],zz[:,1],bins = (100,100), cmap = 'hot')
                self.axes1.imshow(np.rot90(h),interpolation = 'gaussian',extent = ex,cmap = 'hot')
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()
                ...
                self.axes1.clear()
                #axes1.hist2d(zz[:,0],zz[:,1],bins = (100,100), cmap = 'hot')
                self.axes1.imshow(np.log(1+np.rot90(h)),interpolation = 'gaussian',extent = ex,cmap = 'hot')
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()
            elif selected_label == 'index':
                self.axes1.clear()
                self.axes1.scatter(self.z[:,0],self.z[:,1],c = self.lat_cols['index'])
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()
                ...
                self.axes1.clear()
                self.axes1.scatter(self.z[:,0],self.z[:,1],c = self.lat_cols['shift'])
                self.axes1.scatter(self.latent_closest[0],self.latent_closest[1],c = 'r')
                self.lat_canv.draw()
    ...
    def action_changed(self,event):
            ...
            if selected_label == 'click':
                ...
                self.fig.canvas.mpl_disconnect(self.cid)
                self.lasso.disconnect_events()
                self.poly = PolygonSelector(ax = self.axes1, onselect = self.save_starfile, props = self.line,useblit = True)
                self.lat_canv.draw_idle()
    ...
    def onclick(self,event):
        ...
        if self.latent.shape[1]>2:
            ...
            dist = torch.linalg.norm(torch.tensor(self.z)-inst_coord.unsqueeze(0),dim = 1)
        ...
        if self.rep_menu.current_choice == 'volume':
            vol = self.decoder.volume([lat.to(device)],self.r.to(device),self.cons_model.pos.to(device),self.cons_model.amp.to(device),self.cons_model.ampvar.to(device),self.t.to(device)).float()
            self.vol_layer.data = (vol[0]/torch.max(vol[0])).detach().cpu().numpy()
        elif self.rep_menu.current_choice == 'points':
            proj,proj_im,proj_pos,pos,dis = self.decoder.forward([lat.to(device)],self.r.to(device),self.cons_model.pos.to(device),self.cons_model.amp.to(device),self.cons_model.ampvar.to(device),self.t.to(device))
            p = torch.cat([self.nap_zeros.unsqueeze(1),(0.5+pos[0].detach().cpu())*box_size],1)
            self.point_layer.data = torch.stack([p[:,0],p[:,3],p[:,2],p[:,1]],1) 

    def onSelect(self,line):
        path = torch.tensor(np.array(line))
        #path,p_inds = torch.unique(path, sorted = False,return_inverse = True,dim = 0)
        plt.scatter(path[:,0],path[:,1],c = np.linspace(0,1, path.shape[0]))
        plt.show()
        #print(p_inds)
        #print(path)
        ...
        if self.latent.shape[1]>2:
            for j in range(path.shape[0]):
                dist = torch.linalg.norm(torch.tensor(self.z)-path[j].unsqueeze(0),dim = 1)
                inst_ind = torch.argmin(dist,0)
        ...
        if self.rep_menu.current_choice =='volume':
            ...
            for i in tqdm(range(path.shape[0]//2)):
                mu = path[i:i+2].float()
                with torch.no_grad():
                    V = self.decoder.volume([mu.to(device)],self.r.to(device),self.cons_model.pos.to(device),self.cons_model.amp.to(device),self.cons_model.ampvar.to(device),self.t.to(device))
                    if args.pdb_series != None:
                        proj,proj_im,proj_pos,pos,dis = self.decoder.forward([mu.to(device)],self.r.to(device),pdb_pos.to(device),torch.ones(pdb_pos.shape[0]).to(device),torch.ones(n_classes,pdb_pos.shape[0]).to(device),self.t.to(device))
        ...
        if self.rep_menu.current_choice == 'points':
            for i in range(path.shape[0]//2):
                ...
                print(i)
                with torch.no_grad():
                    proj,proj_im,proj_pos,pos,dis = self.decoder.forward([mu.to(device)],self.r.to(device),self.cons_model.pos.to(device),self.cons_model.amp.to(device),self.cons_model.ampvar.to(device),self.t.to(device))
                    poss.append(torch.cat([i*torch.ones(pos.shape[1]).unsqueeze(1).to(device),(0.5+pos[0])*box_size],1))
                    poss.append(torch.cat([(i+1)*torch.ones(pos.shape[1]).unsqueeze(1).to(device),(0.5+pos[1])*box_size],1))
    ...
    def get_part_nr(self,verts):
        ...
        print(np.where(new_indices == True))
    ...
    def save_starfile(self,verts):
        ...
        starfile.write(new_star, args.out_dir+'subset_'+str(self.star_nr)+'_half'+str(args.half)+'.star')
        np.savez(args.out_dir+'subset_'+str(self.star_nr)+'_indices_'+str(args.half)+'.npz',nindsfull)
        self.star_nr += 1
        self.poly.disconnect_events()
        self.poly = PolygonSelector(ax = self.axes1, onselect = self.save_starfile, props = self.line,useblit = True)
...
vis = visualizer(zz,z_lat,decoder,cons_model,V0,cons_volume, nap_cons_pos,dd,amps,lat_colors,dataframe,indices,latent_closest)